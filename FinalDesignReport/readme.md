...
SpotifyStats is web application that combines Spotify analytics with personalized music recommendations, presented through an engaging interface. The application interacts with Spotify's API and the information of a user's profile thorugh a front end and back end component.
The backend consists of two primary components. The main parser will utilize Spotipy, a Python library for Spotify API integration, to handle user authentication and data retrieval. This component will pull user data, clean it, and organize it for distribution to both the frontend visualizations and the recommendation algorithm. The recommendation system is accomplished via a clustering algorithm that clusters users based off of their genre listening distribution, and picks songs from users in a variety of clusters based on the user's distance from those clusters to generate similar, slightly similar, and completely different playlists from the user's tastes.
The frontend will feature two main sections: listening history analytics and music recommendations. The analytics section will showcase interactive visualizations of user listening patterns, while the recommendation section will present playlists generated via the backend algorithm mentioned earlier.
...
Through the combination of detailed analytics, intelligent recommendations, and clean user interface design, SpotifyStats will offer users an innovative platform for exploring their music preferences and discovering new artists aligned with (and outside of) their tastes.
...
Due to deprecations in the Spotify API, our team pivoted from a traditional feature-based recommendation approach to a more robust and scalable **user clustering model**. Instead of relying on Spotify's proprietary audio features (which are no longer broadly available), we represent users based on the **genre distribution** of their top artists and tracks. Each user is encoded as a normalized vector across a predefined set of supergenres; a set of 15 genres mapped up from the niche subgenres that Spotify applies to artists.

We apply **clustering algorithms** (e.g., K-Means) to group users with similar musical profiles. When a new user logs in, we assign that user a cluster and give them options to generate multiple playlists: One from songs of users within their own cluster, one from songs from users in multiple similar clusters, and one from users in distant clusters to promote contrast and exploration. This method allows us to build personalized, yet **data-driven recommendations** without relying on opaque or black-box models; every generated option is clearly explainable.
...
By recommending music from users with **similar tastes**, we increase the likelihood that users will discover music they genuinely enjoy, but may have missed due to algorithmic blind spots or genre overlaps. This user-to-user approach feels more transparent and intuitive than traditional black-box recommenders; users are recommended music from other users that are 'vetted' through similar taste, not through an opaque algorithm that is difficult to ascertain why it is suggesting the songs that it does.

Finally, this clustering system is highly **extensible**. While we used genre distribution as our vector representation, the architecture is abstract enough to support **any distribution-based user profile**; whether it’s based on song attributes, mood tags, or even behavioral patterns. If more niche genres are to be considered, we can also modify and widen the supergenres we consider. This opens up possibilities for applying the same model to entirely different domains outside of music, wherever comparative user data exists.
...
- **Cluster Mates** – Songs that users in the same cluster are listening to, likely overlapping heavily with the user’s current tastes.
   ...
   This visualization showcases a radar chart comparing a user's supergenre distribution (green) with that of their assigned cluster mates (blue). At first glance, there is are certainly some strong similarities, but there are also glaring differences, which at first glance may suggest that this user is not very similar to his cluster. but closer inspection and reasoning reveals why this may be an appropriate cluster.
   
   This user listens to prinarily Hip Hop, Rock, and a small amount of Metal and Country. The songs gnereated from this cluster, however, only includes a strong amount of Hip Hop, a moderate amount of pop, and a small amount of rock. The easiest explanation is that, in our dataset, and in most people's tastes, those that enjoy Hip Hop likely enjoy a good amount of Pop as well. Thus, the user was likely put into this cluster mostly because of his strong affinity to Hip Hop. The result is that the **“Cluster Mates”** generation produces a playlist that does not necessarily encompass the user's taste as a whole, but as a description of where his taste is the strongest.
   
   This example provides a strength of our recommendation algorithm: it compares *users* rather than *music* itself; this user based comparison allows users to get music that is vetted by people they share their strongest tastes with, allowing a blend of music that is familiar to them, along with music that is approved by and listened to others who share a strong genre affinity to them.
